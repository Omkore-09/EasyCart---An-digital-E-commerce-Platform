...
const ProductDetails = () => {
  ...
  const handleTryOnClick = (selectedImagePath) => {
    const url = `http://127.0.0.1:5000/video_feed?shirt_image=${encodeURIComponent(selectedImagePath)}`;
      ...
      <div className="min-h-[200px] flex flex-col lg:flex-row gap-4">
        ...
        <div className="h-96 flex flex-col lg:flex-row-reverse gap-4">
          <div className="h-[300px] w-[300px] lg:h-96 lg:w-96 bg-slate-200 relative p-2">
           ...
           {/** Try-On Button - only show if category is "Cloths" */}
           {(data.category === "Cloths" || data.category === "cloths")  && (
              <button
              className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-3 flex items-center gap-1 hover:bg-red-600"
              onClick={() => handleTryOnClick(data.imagePath)} // Pass the image path of the selected item
            >
              <FaEye />
